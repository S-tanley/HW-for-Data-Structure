...
public class DoubleRecList
{
	...
	private void my_reverse(Node<String> front, Node<String> back) {
		if (front == back) {
			Node<String> tem = front.getNextNode();
			front.setNextNode(front.getPrevNode());
			front.setPrevNode(tem);
		} else {
			Node<String> tem = back.getNextNode();
			back.setNextNode(back.getPrevNode());
			back.setPrevNode(tem);
			my_reverse(front, back.getNextNode());
		}
	}
