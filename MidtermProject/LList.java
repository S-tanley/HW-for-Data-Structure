...
public class LList<T> implements ListInterface<T>
{
    ...
    {
        initializeDataFields();
        Node tem = old.firstNode;
        for (int i = 0; i < old.numberOfEntries; i++) {
            this.add(i+1, tem.getData());
            tem = tem.getNextNode();
        }
    }

    public boolean equals(LList<T> rhs) {
        if (this.numberOfEntries != rhs.numberOfEntries) {
            return false;
        }
        Node tem1 = this.firstNode;
        Node tem2 = rhs.firstNode;
        for (int i = 0; i < this.numberOfEntries; i++) {
            
            if (tem1.getData().equals(tem2.getData())) {
                tem1 = tem1.getNextNode();
